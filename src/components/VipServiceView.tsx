...
export const VipServiceView: React.FC<VipServiceViewProps> = ({ packages, onSelectPackage }) => {
  const categories = [
    {
      id: "confidential",
      title: "重点机要政务公文专递",
      ...
      samplePkg: packages.find(
        (p) => p.vipLevel === "vip_confidential" || p.vipLevel === "vip_government",
      ),
    },
    {
      id: "admission",
              ...
              <div>
                <div className="flex items-start justify-between gap-3">
                  <div className="flex items-center gap-3">
                    <div
                      className={`w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 ${cat.iconColor}`}
                    >
                  ...
                  <span
                    className={`text-[10px] font-bold px-2 py-0.5 rounded-full border flex-shrink-0 ${cat.badgeColor}`}
                  >
                    {cat.badge}
                  </span>
                </div>
